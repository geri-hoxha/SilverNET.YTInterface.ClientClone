...
const FRIENDLY: Record<string, string> = {
  ...
  "project.priorities_not_configured": "This project does not have priorities configured in YouTrack yet.",
...
export function normalizeError(err: unknown): ApiError {
  ...
  if (ax?.isAxiosError) {
    ...
    const friendly = FRIENDLY[code] ?? payload?.message ?? ax.message ?? "Something went wrong. Please try again.";

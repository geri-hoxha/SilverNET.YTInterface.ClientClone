...
import { Bold, Code2, Italic, Link as LinkIcon, List, ListOrdered, Paperclip, Quote, Strikethrough, Type as TypeIcon } from "lucide-react";
...
export function RichTextEditor({
  ...
  useEffect(() => {
    ...
    state.doc.descendants((node, pos) => {
      const ref = node.type.name === "image" ? (node.attrs.attachmentRef as string | null) : node.type.name === "fileAttachment" ? (node.attrs.fileName as string | null) : null;
      if (!ref) return;
...
//
// Deliberately excludes `lastModified` from the key: duplicate File objects
// synthesized from separate clipboard items representing the same image can
// end up with slightly different (or browser-autogenerated) lastModified
// timestamps even though the bytes are identical, which let real duplicates
// slip through this check before.
...
async function insertViaUpload(editor: Editor, file: File, onUploadFile: (file: File) => Promise<string>, previewUrlsRef: React.RefObject<string[]>, at?: number) {
  ...
  try {
    ...
    if (isImage) {
      ...
      editor.chain().focus(at).setFileAttachment({ fileName, fileSize: file.size, previewSrc }).run();
    }
  } catch {
...
function TBtn({ children, label, active, onClick }: { children: React.ReactNode; label: string; active?: boolean; onClick?: () => void }) {
  return (
    <button
      type="button"
      title={label}
      onClick={onClick}
      className={cn("hover:bg-muted hover:text-foreground inline-flex h-7 w-7 items-center justify-center rounded", active && "bg-muted text-foreground")}
    >
      {children}
    </button>
  );
